      ...
      subroutine S17_ruptdirct2012_jrb ( specT, distrup, mag, rupLength, width, ftype, 
...
c       az=90.*pi/180.
       ...
       if (ftype. eq. 0.) then 
         call S17_rupdirct_strike2012 ( mag, 
     1         distrup, theta, Rx, ruplength, specT, X,
     2         slnDird, slnDirn, slnDirp )
       
       else 
       
         call S17_rupdirct_dip2012 ( mag, width, az, 
...
C*********************************************************************************
      subroutine S17_rupdirct_strike2012 ( mag, 
     1           distrup, theta, Rx, ruplength, specT, X,
...
C     -- Find correct coefficient
      ...
      goto 1002
1001     call S24_interp (period(count1),period(count2),cav_ss(count1),
     1                cav_ss(count2),specT,C,iflag)
         call S24_interp (period(count1),period(count2),c1av_ss(count1),
     1                c1av_ss(count2),specT,C1,iflag)
         call S24_interp (period(count1),period(count2),cfn_ss(count1),
     1                cfn_ss(count2),specT,Cn,iflag)
         call S24_interp (period(count1),period(count2),c1fn_ss(count1),
     1                c1fn_ss(count2),specT,C1n,iflag)
         call S24_interp (period(count1),period(count2),cfp_ss(count1),
     1                cfp_ss(count2),specT,Cp,iflag)
         call S24_interp (period(count1),period(count2),c1fp_ss(count1),
...
C-----------------------------------------------------------------------
      subroutine S17_rupdirct_dip2012 ( mag, W, az,
     1           distrup, Rx, ruplength, specT, Y,
     2           lnDird, lnDirn, lnDirp )
...
C     -- Find correct coefficient
      ...
      goto 1002
1001     call S24_interp (period(count1),period(count2),cav_ds(count1),
     1                cav_ds(count2),specT,C,iflag)
         call S24_interp (period(count1),period(count2),c1av_ds(count1),
     1                c1av_ds(count2),specT,C1,iflag)
         call S24_interp (period(count1),period(count2),cfn_ds(count1),
     1                cfn_ds(count2),specT,Cn,iflag)
         call S24_interp (period(count1),period(count2),c1fn_ds(count1),
     1                c1fn_ds(count2),specT,C1n,iflag)
         call S24_interp (period(count1),period(count2),cfp_ds(count1),
     1                cfp_ds(count2),specT,Cp,iflag)
         call S24_interp (period(count1),period(count2),c1fp_ds(count1),
